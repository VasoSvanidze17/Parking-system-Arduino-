...
const byte pin_LED = A2; // LED that turns on when metal is detected
...
void loop()
{
  ...
  for (int imeas = 0; imeas < nmeas + 2; imeas++)
  {
    ...
    if (ledstat == 0) digitalWrite(pin_LED, LOW);
    if (ledstat == 1) digitalWrite(pin_LED, LOW);
    if (ledstat == 2) digitalWrite(pin_LED, HIGH);
  }
  ...
  if (skip > 64)
  { // break off in case of prolonged skipping
    sumsum = sum << 6;
    skip = 0;
  }
